...
import {
    CREATE_FIELDS_CONFIG_URL,
    DELETE_FIELDS_CONFIG_URL,
    GET_ALL_FIELDS_CONFIG_URL,
...
export async function createFieldConfig(
    ...
    try {
        ...
        return {
            success: false,
            error: "Error al crear la configuración del campo",
        };
    }
}
...
export async function updateFieldConfig(
    ...
    try {
        ...
        return {
            success: false,
            error: "Error al actualizar la configuración del campo",
        };
    }
}
...
export async function deleteFieldConfig(
    ...
    try {
        ...
        return {
            success: false,
            error: "Error al eliminar la configuración del campo",
        };
    }
}
...
export async function getAllFieldConfigs(
    user_id: string,
): Promise<AllFieldConfigResult> {
    try {
        const fieldConfigs = await api.get(
            GET_ALL_FIELDS_CONFIG_URL + `?user_id=${user_id}`,
        );
        console.log("[FIELD CONFIGS]", fieldConfigs);
        return {
            success: fieldConfigs.success,
            data: fieldConfigs.data.configs,
            message: fieldConfigs.message,
            timestamp: fieldConfigs.timestamp,
        };
    } catch (error) {
        console.error("Error getting field configs:", error);
        return {
            success: false,
            error: "Error al obtener la configuración del campo",
        };
    }
}
